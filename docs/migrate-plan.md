...
| `browser.tabs.duplicate(tabId, {active, index})` | Chrome accepts only `tabId`, inserts the duplicate after the source, and activates it. The shim duplicates with the native one-argument call, moves the result to `index`, and restores the previously active tab when `active: false`. |
